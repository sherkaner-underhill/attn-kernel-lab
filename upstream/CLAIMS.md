...
gather/centre/rotate/quantize/pack path.**

**Baseline update 2026-08-31:** "every neighbour is dense or ragged and none is
paged" no longer holds. FlashInfer **#4714** (open 2026-08-24) proposes FP8 GQA
*paged* prefill at D256 on SM120 — `page_size` >= 16 over an **FP8 KV pool**,
no transforms, CODEOWNER-blocked on untested D256 and CUDA-graph paths — and
**#3684** (merged) does *paged-storage* NVFP4 with BF16 compute. The precise
combination above (page_size **1**, quantized **compute** over an
**unquantized** pool, transform pipeline) remains unoccupied; see K11's
addendum for the full 2026-08-31 landscape including TRT-LLM's env-gated
source enumeration.
...
| K11 | **[Corrected 2026-08-31 — see the addendum in this cell.]** ~~No public engine runs FP8-class attention at D256 on SM120 at all.~~ cuDNN's FP8 AND block-scale MXFP8 SDPA engines are registered at `d_qk = 128` only — structural in the engine capability table (`frozenset({128})`, cudnn/sdpa/fwd/engines.py, frontend 1.27 / backend 9.20), not version-gated; the 16→256 head range belongs to the f16-class engines. Combined with FlashInfer's D64/D128 NVFP4 surface: our kernel has no same-precision competitor at its head dim, paged or dense. | `probes/cudnn_frost/probe1_pygraph_fp8.py` (refusal reproduced at every shape), engines.py table inspection, 2026-08-30. | measured | strengthens the gap statement (see addendum) **Addendum 2026-08-31:** the absolute form of this claim was falsified by a landscape re-check and is retracted. As of 2026-08-31: cuDNN FROST PR #595 (merged to `develop` 2026-08-17, env-gated, unreleased) widens SM120 FP8 SDPA head dims to 256, dense/ragged only; FlashInfer PR #4714 (open) proposes FP8 GQA *paged* prefill at D256 on SM120 with `page_size` >= 16 over an FP8 KV pool, currently blocked in review on untested D256 and CUDA-graph paths; TensorRT-LLM's fmha_v2 generator enumerates sm120 x e4m3 x D256 x paged-KV in source behind `ENABLE_SM120`, with shipped SM120 cubins reported BF16-only (TRT-LLM #11799). The *released* cuDNN surface (frontend 1.27.0 / backend 9.25.1) remains d128-only for FP8/MXFP8 SDPA, so the engine-table evidence in this row stands for released engines. The claim this row now supports is the narrow one: no public engine SHIPS a VALIDATED paged quantized-compute D256 attention kernel on SM120, and none at page_size 1 over an unquantized KV pool with a centering/rotation transform pipeline. |
