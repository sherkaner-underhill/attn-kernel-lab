...
"""
PER-TENSOR FP8-QK PORT (v0) of NVIDIA's FROST SM120 FMHA prefill template.

Derived from cudnn-frontend 1.27's ``prefill_f16_sm120.py`` (MIT); see
``THIRD_PARTY_NOTICES.md`` and ``licenses/cudnn-frontend-MIT.txt``. Scope,
deliberately narrow (stage 1 of 2): Q/K stored
and MMA'd as E4M3 via ``mma.sync.m16n8k32.e4m3`` (identical C/D layout to
m16n8k16, so softmax/mask/LSE/epilogue are donor-byte-identical); per-tensor
descales fold into ``softmax_scale_log2`` host-side (exact; the SM100 FP8
template documents the same fold); V stored E4M3 for uniform TMA/smem but
dequantized to BF16 in registers, P packs BF16, PV stays m16n8k16.bf16 --
isolating QK quantization as the single variable. Q A-fragments follow the
measured e4m3 A byte layout (claim K1); K B-fragments are swizzled 32-bit smem
loads (correctness-first; ldmatrix is headroom). Original header follows.
...
DTYPE_E4M3 = 0  # frost.tile_dsl.constants: E4M3=0 (upstream validates f16-only)
FP8_QK = PARAMS.dtype_qkv == DTYPE_E4M3
if not FP8_QK:
    validate_params(PARAMS)
elif PARAMS.causal_bottom_right and not PARAMS.is_causal:
    raise ValueError("causal_bottom_right requires is_causal=True")

STORAGE_DTYPE = (cutlass.Float8E4M3FN if FP8_QK else
                 {DTYPE_FP16: cutlass.Float16, DTYPE_BF16: cutlass.BFloat16}[PARAMS.dtype_qkv])
PV_DTYPE = cutlass.BFloat16 if FP8_QK else STORAGE_DTYPE
...
def ptx_mma_m16n8k32_e4m3(
    ...
    """``mma.sync.aligned.m16n8k32.row.col.f32.e4m3.e4m3.f32`` -- identical
    C/D register layout to m16n8k16; doubled K depth in the A/B bytes."""
    return cute.arch.inline_ptx(
...
class SM120FusedMultiHeadAttentionForward:
    ...
    def __init__(
        ...
        if cutlass.const_expr(not FP8_QK) and out_dtype != in_dtype:
    ...
    def _setup_attributes(self):
        ...
        # MMA
        self.qk_mma_k = 32 if FP8_QK else self.MMA_TILER[2]
        self.qk_k_frags = self.kv_tile // self.MMA_TILER[1]
        self.qk_d_frags = self.head_tile // self.qk_mma_k
    ...
    def load_q_tile(
        ...
        if cutlass.const_expr(FP8_QK):
            # e4m3 m16n8k32 A layout (measured; claim K1): thread (r=lane/4,
            # c=lane%4) holds A[r][4c+b] in reg0 byte b, A[r+8] in reg1, and
            # +16 columns in regs 2/3 -- four consecutive bytes per register,
            # one aligned 32-bit load each.
            col0_fp8 = (basic_params.lane % 4) * 4
            col0_in_cta = col0_fp8
            q_regs_offset = 0
    ...
    def mma_qk(
        ...
        if cutlass.const_expr(FP8_QK):
            # e4m3 m16n8k32 B layout (col-major): thread holds B[k=4c+b][n=r]
            # bytes in reg0 and B[k+16] in reg1, with r=lane/4, c=lane%4. In
            # K-tile terms: n is the kv row, k is the head column -- so each
            # register is four CONSECUTIVE head-dim bytes of one kv row: a
            # single swizzled 32-bit smem load. (ldmatrix is optimization
            # headroom; this path is correctness-first.)
            fp8_kv_row_in_frag = basic_params.lane // 4
            fp8_col0 = (basic_params.lane % 4) * 4
            for k_frag in cutlass.range_constexpr(self.qk_k_frags):
                k_row_in_cta = k_frag * 8 + fp8_kv_row_in_frag
                s_off = k_frag * 4
                for d_frag in cutlass.range_constexpr(self.qk_d_frags):
                    b_regs = cutlass.Array(cutlass.Int32, 2, alignment=8)
                    for half in cutlass.range_constexpr(2):
                        k_col_in_cta = d_frag * self.qk_mma_k + fp8_col0 + half * 16
                        ...
                        b_regs[half] = k_smem_ptr.load(count=4, alignment=4).bitcast(cutlass.Int32)[0]
                    q_off = d_frag * 4
                    s_regs[s_off:4] = ptx_mma_m16n8k32_e4m3(
                        q_regs[q_off + 0],
                        q_regs[q_off + 1],
                        q_regs[q_off + 2],
                        q_regs[q_off + 3],
    ...
    def mma_pv(
        ...
        def load_v_frag_pair_fp8(v_frag: cutlass.Constexpr[int], d_frag_pair: cutlass.Constexpr[int]):
            """m16n8k16 bf16 B fragments assembled from E4M3 V bytes.

            f16 B (col-major, k=16, n=8): thread (r=lane/4, c=lane%4) holds
            B[k=2c+{0,1}+8h][n=r] as reg h, for the two n-frags of the pair
            (regs 0,1 = first d-frag halves; 2,3 = second). k is the kv row,
            n is the head column: two loads at CONSECUTIVE kv rows per
            register, each converted E4M3 -> BF16 and packed."""
            regs = cutlass.Array(cutlass.Int32, 4, alignment=16)
            base_kv = v_frag * 16 + 2 * (basic_params.lane % 4)
            for which in cutlass.range_constexpr(2):      # the two n(d)-frags
                d_col = (d_frag_pair * 2 + which) * 8 + basic_params.lane // 4
                v_chunk = d_col // self.tma_swizzle_chunk_elems
                v_col_in_chunk = d_col % self.tma_swizzle_chunk_elems
                for half in cutlass.range_constexpr(2):   # k and k+8
                    row0 = base_kv + half * 8
                    pair = cutlass.Array(PV_DTYPE, 2, alignment=4)
                    for b in cutlass.range_constexpr(2):  # consecutive kv rows
                        v_physical_row = v_chunk * self.kv_tile + row0 + b
                        sV_ptr = (
                            mma_params.sV.data_ptr()
                            + v_physical_row * self.tma_swizzle_chunk_elems
                            + get_swizzled_col(
                        ...
                        pair[b] = PV_DTYPE(cutlass.Float32(sV_ptr.load(count=1)[0]))
                    regs[which * 2 + half] = pack_to_i32((pair[0], pair[1]), PV_DTYPE)
            return regs

        for v_frag in cutlass.range_constexpr(self.pv_v_frags):
            for d_frag_pair in cutlass.range_constexpr(self.pv_d_frags // 2):
                if cutlass.const_expr(FP8_QK):
                    v_vec = load_v_frag_pair_fp8(v_frag, d_frag_pair)
                else:
    ...
    def kernel(
        ...
        sKV = cutlass.Array(
            k.dtype,
            # k.dtype elements; the epilogue aliases this as BF16 O staging, so
            # 1-byte storage needs qo_tile_elems * 2 slots for the O region.
            max(self.kv_tile_elems * 2,
                self.qo_tile_elems * (2 if FP8_QK else 1)),
        ...
        # /////////////////////////////////////////////////////////////////////////////
            ...
            for d_frag_pair in cutlass.range_constexpr(self.pv_d_frags // 2):
                ...
                sO_ptr = sO.data_ptr() + ((compute_warp_idx * (self.pv_d_frags // 2) + d_frag_pair) * (16 * 16) + lane * 8) * (2 if FP8_QK else 1)
                prims.stmatrix(
                    sO_ptr,
                    o_packed,
                    prims.MMALayout.ROW,
            ...
            for d_frag_pair in cutlass.range_constexpr(self.pv_d_frags // 2):
                ...
                if cutlass.const_expr(self.thd_varlen):
                    ...
                    if store_q_seq_idx < seqlen_q and store_col_in_cta < head_dim:
                        ...
                        sO_ptr = sO.data_ptr() + ((compute_warp_idx * (self.pv_d_frags // 2) + d_frag_pair) * (16 * 16) + lane * 8) * (2 if FP8_QK else 1)
                        # 16 staged BYTES regardless of storage width; bitcast is an
                        # identity when storage is already the O dtype.
                        gO_ptr.store(
                            sO_ptr.load(count=16 // self.in_dtype.bytes, alignment=16)
                            .bitcast(o.dtype),
                            alignment=16,
                        )
                else:
...
def compile(  # noqa: A001
    ...
    kernel = SM120FusedMultiHeadAttentionForward(
        ...
        out_dtype=PV_DTYPE if FP8_QK else STORAGE_DTYPE,
    ...
    fake_o = cute.runtime.make_fake_compact_tensor(
        PV_DTYPE if FP8_QK else STORAGE_DTYPE,
        (fake_batch, sq, qh, d),
        stride_order=(3, 2, 1, 0),
        assumed_align=16,
    )
